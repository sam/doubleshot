...
When actually using your packaged library, all you should need it @require "doubleshot/setup"@, which will skip the build process.
...
Using Bundler, and an existing @Gemfile@, the following code will setup or dependencies, and install them on demand if necessary:
...
*NOTE:* Do make sure that @gem "jbundler"@ as the first entry in your @Gemfile@ if you also plan to have _JAR_ dependencies (described below).
